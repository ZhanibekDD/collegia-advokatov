...
const text = {
  ru: {
    ...
    error: "Не удалось загрузить новости. Обновите страницу.",
    newsLabel: "Новость",
  ...
  kk: {
    ...
    loading: "Жарияланымдар жүктелуде…",
    error: "Жаңалықтарды жүктеу мүмкін болмады. Бетті жаңартыңыз.",
    newsLabel: "Жаңалық",
    eventLabel: "Іс-шара",
  },
};
  ...
  useEffect(() => {
    ...
    fetch(NEWS_DATA_URL)
      .then((response) => {
        ...
        return response.json() as Promise<{ posts: NewsPost[] }>;
      })
      .then((result) => { if (active) { setPosts(result.posts); setState("ready"); } })
      .catch(() => { if (active) setState("error"); });
    return () => { active = false; };
  }, []);

  const visible = useMemo(() => posts.filter((post) => filter === "all" || post.kind === filter), [filter, posts]);
          ...
          <div className="news-filter" data-reveal aria-label={locale === "ru" ? "Фильтр публикаций" : "Жарияланым сүзгісі"}>
            {(["all", "news", "event"] as const).map((value) => (
              <button type="button" className={filter === value ? "active" : ""} aria-pressed={filter === value} onClick={() => setFilter(value)} key={value}>
                ...
                <strong>{value === "all" ? posts.length : posts.filter((post) => post.kind === value).length}</strong>
              </button>
            ))}
          </div>

          {state === "loading" && <div className="status-panel"><span className="spinner" />{t.loading}</div>}
          {state === "error" && <div className="status-panel error"><Newspaper />{t.error}</div>}
